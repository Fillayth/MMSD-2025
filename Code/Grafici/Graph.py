...
class Graphs:
    ...
    def _create_limit_line_shape(self, x0: float, x1: float, y: float, color: str = "red") -> dict:
    ...
    def _get_free_time_per_day(self, patients: list, day: int, metric: str = "eot") -> float:
    ...
    def BoxPlotUnusedTime(self, weeks: PatientListForSpecialties, title: str) -> None:
        ...
        for op, patients in weeks.items():
            ...
            last_week = max((p.opDay for p in patients), default=0) // Settings.week_length_days
    ...
    def PrintDailyBoxGraph_withEOTplanned(self, operation: PatientListForSpecialties, baseTitle: str, plan_eot: dict | None = None, use_rot_as_primary: bool = False):
        ...
        for op, patients_real in operation.items():
            ...
            for weekNum in range(num_weeks):
                ...
                for day in range(weekNum * Settings.week_length_days, (weekNum + 1) * Settings.week_length_days):
                    for room_id in range(Settings.workstations_config[op]):
                        ...
                        secondary_label = "EOT (pianificato)" if use_rot_as_primary else "ROT (reale)"
                        primary_label = "ROT (reale)" if use_rot_as_primary else "EOT (pianificato)"
                        ...
                        if use_rot_as_primary:
                            ...
                            for p in real_day_room:
                                fig.add_trace(go.Bar(
                                    ...
                                    y=[p.rot],
                                    name=f"Patient {p.id}",
                                    text=[f"Patient {p.id}: {int(p.rot)}m {int((p.rot % 1) * 60)}s"],
                            ...
                            if plan_day_room is not None:
                                for pp in plan_day_room:
                                    ...
                                    fig.add_trace(go.Bar(
                                        ...
                                        text=[f"Patient {pid} EOT: {int(peot)}m {int((peot % 1) * 60)}s"],
                                        hovertemplate=f'Paziente {pid}<br>EOT: {peot:.2f} min<br>D:{pday}|MTB:{pmtb}<extra>EOT (pianificato)</extra>',
                                        marker=dict(color=color_map_progressive.get(pid, "gray"), opacity=0.3),
                                        cliponaxis=True,
                                        textposition='inside',
                                        offsetgroup="back",
                                        offset=-0.2,
                                ...
                                for p in real_day_room:
                                    fig.add_trace(go.Bar(
                                        ...
                                        text=[f"Patient {p.id} EOT: {int(p.eot)}m {int((p.eot % 1) * 60)}s"],
                                        hovertemplate=f'Paziente {p.id}<br>EOT: {p.eot:.2f} min<br>D:{p.day}|MTB:{p.mtb}<extra>EOT (pianificato)</extra>',
            ...
            metric_text = "ROT = barra piena | EOT = barra trasparente" if use_rot_as_primary else "EOT = barra piena | ROT = barra trasparente"
            fig.add_annotation(
                x=0.01, y=1.08,
                ...
                for day in range(weekNum * Settings.week_length_days, (weekNum + 1) * Settings.week_length_days):
                    for room_id in range(Settings.workstations_config[op]):
                        ...
                        for p in real_day_room:
                            ...
                            label = "ROT (reale)" if use_rot_as_primary else "EOT (pianificato)"
                            ...
                            fig.add_trace(go.Bar(
                                ...
                                y=[y_val],
                                name=f"Patient {p.id}",
                                text=[f"Patient {p.id}: {int(y_val)}m {int((y_val % 1) * 60)}s"],
                        ...
                        # --- BACK: Metrica secondaria (trasparente con offset) ---
                            ...
                            label_sec = "EOT (pianificato)" if use_rot_as_primary else "ROT (reale)"
                            ...
                            fig.add_trace(go.Bar(
                                ...
                                text=[f"Patient {p.id} {label_sec.split()[0]}: {int(y_val_sec)}m {int((y_val_sec % 1) * 60)}s"],
    ...
    def PrintTrendLineGraph(
        ...
        for op, patients in operation.items():
            ...
            for d in range(last_day):
                for room_id in room_ids:
                    daily_patients = [p for p in patients if p.workstation == room_id + 1 and p.opDay == d]
                    time_metric = (
                        sum(p.rot for p in daily_patients) if use_rot_as_primary
    ...
    def MostraTabellaConfrontoPlotly(self, scenari: dict) -> None:
        ...
        for nome_scenario, operation_data in scenari.items():
            for specialty, patients_real in operation_data.items():
                ...
                giorni_presenti = [p.opDay for p in patients_real if hasattr(p, 'opDay')]
                ...
                priorita_media = (somma_giorni_attesa / tot_pazienti) if tot_pazienti > 0 else 0.0
                
                # Aggiungi riga alla tabella
                nomi_scenari.append(nome_scenario)
                specialta_nomi.append(specialty)
                pazienti_n.append(tot_pazienti)
                tempo_max_settimana = week_length_days * Settings.daily_operation_limit * num_rooms + Settings.weekly_extra_time_pool * num_rooms
                tempi_d_medi.append(f"{tempo_medio_settimanale:.2f}/{tempo_max_settimana}")
                priorita_p.append(f"{priorita_media:.2f}")

        # Crea tabella Plotly
        fig = go.Figure(data=[go.Table(
            header={
                "values": ['<b>Scenario</b>', '<b>Specialità</b>', '<b>N.Pazienti</b>',
                           '<b>Utilizzo OP (min/week)</b>', '<b>Attesa Media (giorni)</b>'],
                "fill_color": '#1f77b4',
                "align": 'center',
                "font": {"size": 12, "color": 'white'}
            },
            cells={
                "values": [nomi_scenari, specialta_nomi, pazienti_n, tempi_d_medi, priorita_p],
