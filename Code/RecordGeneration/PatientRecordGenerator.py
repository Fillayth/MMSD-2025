...
def sample_from_distribution(
    distribution, return_type, mean=None, std=None, shape=None, scale=None
):
    ...
    if distribution == "lognormal":
        ...
        raise ValueError(
            "Distribuzione non supportata. Scegli tra 'lognormal', 'gamma', 'weibull', 'normal', 'poisson'."
        )
...
def generate_time(K2_params, K7_params, K8_params, K9_params, K3_params):
    ...
    t_K2 = sample_from_distribution(
        K2_params["distribution"],
        float,
        mean=K2_params.get("mean"),
        std=K2_params.get("std"),
        shape=K2_params.get("shape"),
        scale=K2_params.get("scale"),
    )
    t_K7 = sample_from_distribution(
        K7_params["distribution"],
        float,
        mean=K7_params.get("mean"),
        std=K7_params.get("std"),
        shape=K7_params.get("shape"),
        scale=K7_params.get("scale"),
    )
    t_K8 = sample_from_distribution(
        K8_params["distribution"],
        float,
        mean=K8_params.get("mean"),
        std=K8_params.get("std"),
        shape=K8_params.get("shape"),
        scale=K8_params.get("scale"),
    )
    t_K9 = sample_from_distribution(
        K9_params["distribution"],
        float,
        mean=K9_params.get("mean"),
        std=K9_params.get("std"),
        shape=K9_params.get("shape"),
        scale=K9_params.get("scale"),
    )
    t_K3 = sample_from_distribution(
        K3_params["distribution"],
        float,
        mean=K3_params.get("mean"),
        std=K3_params.get("std"),
        shape=K3_params.get("shape"),
        scale=K3_params.get("scale"),
    )
...
def write_reports(
    patient_records, weekly_report, seed, project_root
) -> tuple[str, str]:
...
def generate_csv(
    ...
    for week in range(num_weeks):
        ...
        for weekday in range(1, Settings.week_length_days + 1):
            ...
            people_params = Settings.daily_patient_arrival_distribution_params.get(
                people_distribution, {}
            )
            ...
            for _ in range(patients_today):
                ...
                prio_params = priority_params[
                    operation_type
                ]  # Ottieni i parametri di priorità per la specialità
                ...
                real_time = round(
                    estimated_time * np.random.uniform(rot_min, rot_max), 3
                )
                priority = sample_from_distribution(
                    prio_params["distribution"],
                    int,
                    mean=prio_params["mean"],
                    std=prio_params["std"],
                )
        ...
        weekly_report.append([week + 1, round(used_minutes, 2), patients_in_week])
